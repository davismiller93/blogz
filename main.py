...
def home():
    ...
    return render_template('home.html', title= "The Blog", 
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if username == "":
            error["name_error"] = "Please enter a username"
        if password == "":
            error["pass_error"] = "Please enter a password"
        elif len(password) < 2:
            error["pass_error"] = "Password must be more than two characters long"
        else:
            if password != verify:
                error["verify_error"] = "Paswords must match"

        existing_user = User.query.filter_by(username=username).first()
        if existing_user:
            error["name_error"] = "This username is already in use"
    ...
    return render_template("signup.html", title= "signup for this Blog",
